...
const emit = defineEmits(['back', 'experiment-update'])
...
const exportTopologyData = () => {
  ...
  a.href = url
  a.download = `topology_${props.experimentId}_round_${currentRound.value}.json`
...
const exportMetricsData = () => {
  ...
  const a = document.createElement('a')
  a.href = url
  a.download = `metrics_${props.experimentId}.json`
  document.body.appendChild(a)
  a.click()
  document.body.removeChild(a)
  URL.revokeObjectURL(url)
...
onMounted(() => {
  ...
  // 如果实验正在运行，自动连接实时模式
  if (experimentInfo.value.status === 'running') {
    toggleLiveMode()
  }
})
