    ...
    >
      <div v-if="selectedNode">
        <a-descriptions :column="1" bordered size="small">
          ...
          <a-descriptions-item label="参与训练" v-if="selectedNode.layer === 'L0'">
...
const generateTopologyData = () => {
  ...
  for (let clusterId = 1; clusterId <= currentParams.value.numClusters; clusterId++) {
    ...
    for (let j = 1; j <= numClients; j++) {
      ...
      const participationRate = clusterId === 1 ? 0.55 : clusterId === 2 ? 0.67 : 0.61 // 参与率适当分散
      const isParticipating = isOnline && seededRandom(participationSeed) < participationRate
  ...
  return [
    ...
    // L2 - 代理样式（菱形，清新簇颜色）
    {
      ...
      style: {
        ...
        'box-shadow': (ele) => {
          const clusterId = ele.data('clusterId')
          const colors = ['rgba(107, 163, 208, 0.25)', 'rgba(149, 212, 117, 0.25)', 'rgba(255, 193, 7, 0.25)']
  ...
  const getEdgeDefaultStyle = (edge) => {
    ...
    switch (type) {
      ...
      case 'full-connect-disappearing':
        originalWidth = type === 'full-connect' ? 0.8 :
                         type === 'full-connect-warning' ? 1 : 1
        originalOpacity = type === 'full-connect' ? 0.6 :
                          type === 'full-connect-warning' ? 0.6 : 0.3
        break
      case 'inter-cluster':
        originalWidth = 2
        originalOpacity = 0.5
          ...
          complete: () => {
            ...
            proxy.animate({
              style: {
                ...
                'width': 50,
                'height': 50,
                'box-shadow': `0px 3px 12px rgba(${clusterId === 1 ? '107, 163, 208' : clusterId === 2 ? '149, 212, 117' : '255, 193, 7'}, 0.25)`
              }
            }, {
              duration: 500
            })
  ...
  nodes.forEach(nodeData => {
    ...
    if (existingNode.length > 0) {
      ...
      if (nodePositions[nodeData.data.id]) {
        cy.getElementById(nodeData.data.id).position(nodePositions[nodeData.data.id])
      }
    }
  })
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (round >= 0) {
      logs.push({
        ...
        message: `轮次 ${round}: 模型更新完成，损失：0.${Math.floor(Math.random() * 300 + 150)}`,
...
.topology-container::before {
  ...
  background: 
    ...
    radial-gradient(circle at 40% 60%, rgba(250, 140, 22, 0.03) 0%, transparent 50%);
  pointer-events: none;
}
