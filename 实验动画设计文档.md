...
| L2 | **Cluster Proxy (i = 1…K)** | 同行 K 个节点 | 向簇内广播模型、做簇内 FedAvg，每轮从client cluster中选取若干节点作为参与本轮训练的客户端       |
...
* 服务器为每个客户端 $i$ 随机初始化本地模型参数 $\theta_i^0$
* 同时初始化共享层权重 $\tilde{w}^0$，作为全局参考

---

### **1. 服务器广播共享层**

* 在第 $t$ 轮开始时，服务器向所有客户端广播当前共享层参数：
...
* 每个客户端将自己模型中的对应共享部分替换为服务器下发的共享层：

  $$
  \tilde{\theta}_i^t \leftarrow \text{Replace}(\theta_i^{t-1}, \tilde{w}^{t-1})
  $$
...
* 客户端在本地数据 $x_i$ 上执行 $E$ 步 SGD 更新本地模型：

  $$
  \theta_i^{t} \leftarrow \tilde{\theta}_i^t - \eta_\theta \nabla \mathcal{L}_c(\tilde{\theta}_i^t; x_i)
  $$
* 其中，$\mathcal{L}_c$ 是分类交叉熵损失，$\eta_\theta$ 是学习率

---

### **4. 客户端上传共享层**

* 客户端提取其本地模型中定义为共享部分的权重 $w_i^t \subset \theta_i^t$ 并上传至服务器：

  $$
  w_i^t \rightarrow \text{Server}
  $$
...
* 构建多维迹范数正则项作为任务间关联建模方式：

  $$
  \mathcal{L}_r = \sum_{l=1}^L \sum_{k=1}^{p} \left\| W_{(k)}^l \right\|_*
  $$
* 其中 $W_{(k)}^l$ 是张量在第 $k$ 模下的 unfolding 矩阵

#### **(c) 次梯度下降优化共享层**

* 对于每个 unfolding 矩阵迹范数的次梯度为：
...
* 对每个客户端上传的共享切片，执行一次梯度下降：
  ...
  $$
* 更新后的 $\tilde{w}_i^t$ 是服务器针对客户端 $i$ 个性化后的共享层

---

### **6. 服务器回传共享层**

* 将每个客户端的更新共享层 $\tilde{w}_i^t$ 发送回客户端，供下轮训练替换用

---

## 优化目标（全局视角）

FedSAK 等价于在以下全局优化目标上迭代：

$$
\min_{\Theta} \left\{
\frac{1}{M} \sum_{i=1}^M \frac{1}{n_i} \sum_{j=1}^{n_i}
\mathcal{L}(F_i(\Theta; x_{ij}), y_{ij})
+ \lambda \|W\|_*
\right\}
$$
